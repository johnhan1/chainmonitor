...
class DefaultPairQualityPolicy(PairQualityPolicy):
    ...
    def is_acceptable(self, pair: NormalizedPair, chain_id: str) -> bool:
        ...
        if dex_id and dex_id in self._settings.dex_blacklist_ids_set:
            return False
        route_signal = f"{pair.pair_address.lower()}|{pair.url.lower()}"
        keywords = self._settings.route_blacklist_keywords_tuple
        if any(keyword in route_signal for keyword in keywords):
            return False
        if pair.pair_created_at_ms is None:
            return False
        age_seconds = time() - (pair.pair_created_at_ms / 1000.0)
        min_age_seconds = self._settings.get_min_pair_age_seconds(chain_id=chain_id)
        ...
        if pair.volume_5m < 0 or pair.liquidity_usd < 0 or pair.buys_5m < 0 or pair.sells_5m < 0:
            return False
        max_ratio = self._settings.get_max_volume_liquidity_ratio(chain_id=chain_id)
