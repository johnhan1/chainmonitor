...
def test_settings_parse_chain_token_addresses() -> None:
    settings = ChainSettings(bsc_token_addresses="BNB=0x111,CAKE=0x222")
...
def test_settings_parse_required_address_symbols() -> None:
    settings = IngestionSettings(required_address_symbols_by_chain="bsc=BNB|CAKE,eth=*")
    assert settings.get_required_address_symbols("bsc") == {"BNB", "CAKE"}
    assert settings.get_required_address_symbols("eth") == {
        "ETH",
        "USDC",
        "WBTC",
        "PEPE",
...
def test_settings_get_geckoterminal_network(monkeypatch) -> None:
    monkeypatch.setenv("CM_MARKET_DATA_GECKOTERMINAL_NETWORK_BY_CHAIN", "bsc=bnb-smart-chain")
...
def test_settings_get_birdeye_chain(monkeypatch) -> None:
    monkeypatch.setenv("CM_MARKET_DATA_BIRDEYE_CHAIN_BY_CHAIN", "bsc=bsc-mainnet")
    get_ingestion_settings.cache_clear()
    settings = get_chain_settings()
...
def test_settings_rate_limit_resolves_by_provider_then_chain() -> None:
    settings = IngestionSettings(
        ...
        rate_limit_per_second_by_chain="bsc=9.0",
        rate_limit_per_second_by_provider="dexscreener=4.0",
        rate_limit_per_second_by_provider_chain="dexscreener:bsc=3.5",
    )
    assert settings.get_rate_limit_per_second("bsc", provider="dexscreener") == 3.5
    assert settings.get_rate_limit_per_second("eth", provider="dexscreener") == 4.0
    assert settings.get_rate_limit_per_second("bsc", provider="unknown") == 9.0


def test_settings_rate_limit_capacity_resolves_by_provider_then_chain() -> None:
    settings = IngestionSettings(
        rate_limit_capacity=20,
        rate_limit_capacity_by_chain="bsc=18",
        rate_limit_capacity_by_provider="dexscreener=8",
        rate_limit_capacity_by_provider_chain="dexscreener:bsc=7",
    )
    assert settings.get_rate_limit_capacity("bsc", provider="dexscreener") == 7
    assert settings.get_rate_limit_capacity("eth", provider="dexscreener") == 8
    assert settings.get_rate_limit_capacity("bsc", provider="unknown") == 18
...
def test_source_strategy_factory_uses_configured_strategy_order(monkeypatch) -> None:
    ...
    get_chain_settings.cache_clear()
